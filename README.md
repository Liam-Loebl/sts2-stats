...
- **Sanity report + verifier.** `import_all.py` ends with a topline report (run counts, win rates, per-character / per-schema / per-build splits) I eyeball after every re-import. For deeper checks, `verify.py` runs a battery of invariants — no NULLs in required columns, FK integrity, co-op local-user resolution on every co-op run, floor math against source JSON, random whole-row spot-checks, and idempotency — and exits non-zero if anything fails. It also runs a soft tone scan on README and SPEC for common AI-prose tells (em-dash overuse, "not just X" phrasing, promotional vocab, meta-commentary, inflated symbolism, conjunctive sentence-starters) and surfaces them as warnings to re-read. So when the game ships a v10 schema or my prose drifts I see it immediately.
